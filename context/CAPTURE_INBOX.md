...
- ❓ Open questions still needed before building:
  - Alex loan: **fully specified.** Starting balance = $210 (approximate placeholder, see above). Deduction = percentage of the invoice, scaling from **2.5% at a $100 invoice up to 5% at a $500+ invoice** (built as a linear scale between those two points — flagging this as my assumption since Brett gave the two endpoints, not the curve between them; invoices under $100 default to the 2.5% floor rate unless corrected). **No deduction at all on invoices $99.99 and below** — the scale only applies to $100+ invoices. **Cap: $25 max deduction per invoice**, regardless of what the percentage formula would otherwise produce. **Rounding: nearest $2.50** — round DOWN for invoices under $350, round UP for invoices $351 and up (no stated rule for exactly $350 — will treat as round-down, the same side as "under $350," unless corrected). Final payoff payment is never rounded — pays the exact remaining balance.
