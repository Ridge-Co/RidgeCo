...
- **FOLLOW-UP (July 23) — the QUALITY dimension.** Brett: "not breaking anything is important but vague and lacks a quality parameter. What else can we add so the product is high quality?" Key reframe: the substrate (smoke + validator + rollback) only guarantees **regression safety — a FLOOR** ("don't break what worked"). It says nothing about whether the NEW thing is *good*. Quality is a separate, POSITIVE standard the output must MEET, and it has to be defined, tested-for, reviewed, measured, and calibrated — 5 verbs: **DEFINE** (B-144 Quality Bar/DoD rubric — the actual "quality parameter" he's asking for), **TEST positively** (B-145 golden-path scenario tests + seeded fixtures, not just 200-OK smoke), **REVIEW** (B-146 multi-lens quality-reviewer agent beyond the built-vs-brief validator + B-149 house-style consistency checks), **MEASURE** (B-147 quality KPIs into the Optimizer + B-148 canary/auto-rollback), **CALIBRATE** (B-150 merge-threshold policy + 1-in-N Brett spot-checks). Grounded in his real quality debts: HUB_UX_DESIGN_FOUNDATION audit (status defined ~22 ways, 3 invoice paths, 5 button systems), vendor Spanish P0 ~95% dead, money/QB penny-correctness, B-093 security. → B-144..B-150.
- **FOLLOW-UP #2 (July 23) — the KEYSTONE realization.** Brett: "I don't know how to code or analyze it, so I need something working FOR me to check everything." This flips the whole stack: B-140..150 are all machine-facing — invisible/useless to a non-coder. The missing piece is a **human-facing Control Panel** that translates every gate/test/KPI into plain English + an approve/reject decision. **Brett = the APPROVER, not the inspector** (reads an inspection report, never source code). → **B-151** (keystone). Screens: Hub Health (smoke → "5 core jobs working"), **Needs You** approval queue (plain summary + quality report card + "how I checked it" + Preview/Approve/Send-back), Did-For-You (autonomous low-risk log), Watch List (KPIs in English incl. Brett-corrections/feature + EN/ES %), one-tap **roll back to this morning**. Interactive MOCKUP delivered this session (`brettos-control-panel.html`), mobile-first Fairfax look — awaiting Brett feedback before it graduates to a real build wired to live data. Insight to carry: for Brett, the control surface IS the product; the quality machinery only matters if it renders here.
- **FOLLOW-UP #3 (July 23) — command center + the sandbox decision.** (1) Brett: "use this for everything, not just the sites — see what's open, what's important, what's next." → B-151 scope EXPANDED to a full **command center** with a **Today** home tab across all ventures (unifies B-016 multi-venture dashboard + B-051 daily digest into the trusted surface). v2 mockup delivered. (2) Brett's sandbox Q (he explicitly asked NOT to build overkill): does a test-customer/test-vendor/test-Sheet/test-payments system add value, or does B-140+B-145 already cover it? **Verdict logged as B-152: ~70% already covered** (test Sheet=B-145, isolated runtime=B-140). Real gap = the **QuickBooks path**. **KEY CORRECTION to Brett's instinct:** do NOT create test entities in the LIVE QB company (pollutes real books/reconciliation/taxes) — instead **mock QB by default** + use **Intuit's free QB Sandbox company** for money-path golden tests + a hard **"sandbox mode" flag** so the preview Worker structurally can't touch live Sheet/QB. **Scope to money/QB/customer paths ONLY** (for UI/capture, B-145's test Sheet already suffices — more there = the overkill he wants to avoid). Payoff justifying it: the money→books path reaches his approval already exercised end-to-end = a real quality-score jump, not ceremony.

### CAP-030 — BarrelCo delivery bottleneck → priority-engine idea (multi-venture — event + feature)
- Raw: Brett, re the control panel: "I don't see a way to switch between ventures or sort/filter. It's good if they distill to the top — what's most important for THIS venture / next steps for this venture — and then distill to what's most important overall. Ridge Co always floats to top because everything I do there has someone waiting on me (customer waiting on estimate/invoice/completed job; vendor waiting on payment/info). But I can't drop balls on the other ventures either. Example: barrels going out to Community Forklift this week — huge bottleneck because my truck needed work to tow a trailer safely. That's fixed, drop-off scheduled Friday. But I lost ~2 solid weeks of sales; probably ~$2,000 opportunity cost, definitely $1,500."
- Type: event + feature idea
- Status: linked → graduated (B-153; panel v3 delivered)
- **The event (durable lesson):** FLEET readiness is a hard dependency for BARRELCO fulfillment. A tow-capacity truck repair stalled the Community Forklift consignment delivery ~2 weeks → lost personal + CF sales. Now resolved; CF drop-off scheduled **Friday**. Cross-venture dependency (Fleet ↔ BarrelCo) is exactly the kind of thing that must be VISIBLE early. ($ opportunity-cost figure kept in private `barrelco.md`.)
...
### CAP-036 — Sticky-note + voice-memo brain-dump, Sep 24 2026 (RidgeCo Hub — mostly NEW, active)
- Raw: 3 handwritten sticky notes + a walkthrough voice memo covering messaging/notification UX, vendor task tracking, vendor loan tracking, billing sub-statuses, and a late-night brainstorm list of bugs/features.
- Type: mixed (bug fixes, features, personal to-dos, decisions)
- Status: new — approved items below, ready to build; a few open questions remain
- Sub-items (Brett said "go ahead" on all unless flagged):
  1. Photo-request SMS template + button on the WO, to the tenant.
  2. Number-entry → button-select for choosing SMS recipients — fix **everywhere** in the Hub this pattern exists, not just the vendor-reassignment flow.
  3. Suppress "vendor reassigned" SMS to the vendor when the WO is already marked complete — folds into #4 below.
  4. **Notification-matrix deep dive** (Brett's own framing): map every status-change / reassignment / schedule-change combination against who (tenant/vendor/owner) needs to be notified vs. not, so the system defaults correctly instead of over-notifying. Concrete case that surfaced it: reassigning a completed job to himself while standing in the property with the tenant — tenant doesn't need a "reassigned" text, vendor doesn't need a "marked complete" text, but if reassigned BEFORE the job was done, the vendor DOES need to know so they don't show up. Ties directly into #10 (runtime SMS checkbox at mark-complete) — Brett wants smart defaults, with the ability to override at runtime, not a blanket always-ask.
  5. Whole-property multi-unit WO: clarify what the tenant-SMS checkbox actually does, and default it to unchecked (don't message all tenants when the WO is for common areas / the whole building).
  6. Delivery feature: route the tenant relay through Twilio SMS + email (not the current draft-to-phone's-native-messaging-app behavior).
  7. Delivery feature: store field gets a dropdown (Home Depot, Lowe's, Ace Hardware, Walmart, expandable later) with a free-type fallback — must NOT break the existing/planned automated delivery-notice population path.
  8. Delivery feature: selecting a property should prefill the delivery address (currently doesn't).
  9. Property Notice page bug: owner/property info doesn't refresh when a different property is selected.
  10. Runtime SMS-recipient checkbox (tenant/vendor/owner) shown specifically at the mark-complete transition, defaulting to the system's normal auto-send rules — reinforces/depends on #4.
  11. Itemized receipts on Review Bills — show each receipt inside a bill (not just the total), including how each was coded (reimbursed-to-vendor vs. billed-to-company). Scope confirmed: Review Bills + vendor portal only (the WO detail view already shows this, no change needed there). Rationale: catch problems before the bill goes to QuickBooks/the owner — no way to unwind after it's sent.
  12. **Vendor task-tracking / "needs attention" system.** Request types (with canned messages): Request Photos, Request Description Update/Clarification, Other (free-typed at send time). Triggered from the WO or the bill-review section in the Hub. Surfaces in a dedicated, prominent section of the **vendor portal** (not buried in the normal WO view) so the vendor can address it. The request should specify whether it's about their **estimate** or their **invoice** (both are inherently tied to the WO already) — that's the only scoping needed, not a separate WO-vs-invoice attachment model. Vendor marks done → flows to a "needs your review" queue for Brett (not auto-trusted — he does a manual check).
  13. **Vendor loan/advance tracking — general mechanism, not just Alex/Gina-specific.**
      - **Gina**: paid via Venmo (bypasses the normal invoice-payment system on purpose — she needs fast payment). Wants a simple ledger inside the Hub: manual entries with amount + date, going **both directions** (deductions AND new loan advances — her balance can go up, e.g. a recent +$100 advance).
      - **Alex**: repaid via automatic deduction from each invoice payment, **percentage-based**, rounded to the nearest $2.50 (simple rounding, no cents). **Exception: the final payoff payment is NOT rounded — pays the exact remaining balance.** There will be a cap (see open question below).
      - General: build this as a reusable vendor-loan-balance feature (any vendor, not hardcoded), since Brett said "Alex and every other vendor."
  14. New **"Invoiced — Pending Info"** sub-status: for the vendor's benefit (so they know something's needed before they get paid) AND for Brett's — needs its own filter on the vendor portal's Invoiced tab (paid invoices collapse below it) and on Review Bills (so blocked bills are visibly held back from accidentally going out). Applies to vendor-submitted invoices; Scope Proposal milestone billing scope not yet decided (see open question).
  15. **Bug — bill disappeared from Review Bills after pressing Cancel at final QuickBooks-send confirmation.** Real incident: Eddie Smith's bill. Brett reached the last "confirm send to QuickBooks" step, pressed Cancel instead, and the bill never reappeared on Review Bills. Needs (a) recovering that specific bill if it's actually stuck somewhere, (b) a fix so Cancel returns the bill to the list instead of removing it, (c) a check for whether other bills hit the same fate. **Treat as high priority — money/billing visibility bug.**
  16. **Status-model rework (concept, not yet scoped as a build):** rename/expand the WO status set for clarity — e.g. "Estimate Requested" → "Estimate Completed" → "Work In Progress" → "Work Completed" — so a vendor who submits an estimate but marks the job "Work Completed" without ever getting the estimate approved or submitting an invoice can be caught and sent a short, concise SMS asking to clarify (is the job done? did you mean to submit an invoice?). This is the system-side backstop for the Juana/Cesar estimate-vs-invoice confusion (see personal to-do below). **Needs its own design pass** — touches the core status model used across owner/tenant/admin views.
  17. Vendor active/inactive: build a self-service mechanism in the Hub — select a vendor, deactivate/reactivate directly — instead of doing it by hand each time. (Need to confirm whether an Active/Inactive field+toggle already exists on the Vendor edit modal before building new.)
  18. Deactivate vendors **Emmanuel Tire(s)** and **Brian Furr** — approved, executing now.
- Personal to-dos (not builds, but Brett wants a mechanism):
  19. Talk to Juana and Cesar about using the Invoices section, not Estimates, once work is actually complete. Brett doesn't want to over-message them — wants the system-side status rework (#16) to eventually catch this automatically with a concise SMS, rather than a manual conversation alone. **This exact pair (#19↔#16) is the motivating example in `context/TASK_LINKING_BUILD_BRIEF_v1.0.md` (Sep 24 2026) — the design for how a one-off like this gets linked to the build that would close it, and how Claude should proactively offer to execute it (Brett approving per instance) once #16 ships.**
  20. Follow up with Alex — possible missing receipt on a pending invoice (enough labor/work implied that a receipt may be missing; he has Brett's Lowe's card).
- Late-night brainstorm bugs/features (separate from the sticky notes, same session):
...
- ❓ Open questions still needed before building:
  - Alex loan: **fully specified.** Starting balance = $210 (approximate placeholder, see above). **Deduction bases entirely on the LABOR portion of the invoice, never the materials/reimbursement portion** — e.g. a $200 invoice made of $100 materials + $100 labor only runs the $100 labor figure through the formula below; the materials reimbursement passes through untouched. Rationale (Brett's own): materials spend isn't within Alex's control, so scaling the deduction off the full invoice would unfairly speed up repayment based on something he doesn't control — the loan is meant to be repaid at a fair, controlled pace, not as fast as possible. Deduction = percentage of that labor figure, scaling from **2.5% at $100 of labor up to 5% at $500+ of labor** (built as a linear scale between those two points — flagging this as my assumption since Brett gave the two endpoints, not the curve between them). **No deduction at all when labor is $99.99 and below** — the scale only applies to $100+ of labor. **Cap: $25 max deduction per invoice**, regardless of what the percentage formula would otherwise produce. **Rounding: nearest $2.50** — round down for labor under $350, round UP for labor $351 and up (no stated rule for exactly $350 — will treat as round-down, the same side as "under $350," unless corrected). Confirmed by Brett: this threshold, like the rest of the formula, is measured against the LABOR figure, not the full invoice total. Final payoff payment is never rounded — pays the exact remaining balance.
