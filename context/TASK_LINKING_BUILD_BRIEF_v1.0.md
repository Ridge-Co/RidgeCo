...
**Direct answer, no hedging:** use structured FKs as the primary link, reusing the Notes schema exactly (`Property_ID, Unit_ID, WO_ID, Owner_ID, Vendor_ID, Tenant_ID` + add `Venture`). Also add one free-text `Tags` column alongside it — not as a "promote later" staging area, but as a permanent, orthogonal field for things a FK can't express (theme/type words like `recurring`, `personal`, `high-value`, `blocked-on-X`).

Why not "loose tags now, promote to FKs later": that promotion step is a second pass Brett will not reliably come back and do — it's exactly the kind of janitorial task that's the subject of this whole conversation. A tag ("Cesar") that should have been a Vendor_ID FK just becomes another thing that needs reconciling later, the same disconnect Brett is trying to fix. The capture-time cost of picking a real FK instead of typing a name is close to zero, because Brett is virtually always looking at a specific property/vendor/WO/tenant when the task originates — the picker is right there. Loose tags are the right tool only for genuinely un-FK-able dimensions (mood/theme/category), never as a stand-in for "I'll link it correctly eventually."

**Bottom line:** structured FK is mandatory at capture whenever the task is about a real Property/Unit/WO/Owner/Vendor/Tenant. `Tags` is additive, not a fallback.
