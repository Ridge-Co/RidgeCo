...
**Version:** v1.13 | **Last Updated:** September 2, 2026
...
## SECTION 6: PATTERN LIBRARY (PAT-001 through PAT-033)
...
**PAT-031: ROUTE + INSTRUMENT BY DEFAULT — MANDATORY**
Every build that calls AI must route the work by tier through the model router (`routeAI`), not call an expensive model directly. Cheap tier by default (Gemini Flash / Haiku), escalate to Claude Sonnet/Opus only when the job earns it or the cheap result fails validation. Never route business data to an untrusted/third-party "free" provider — direct-to-provider from our own Worker only (see `MODEL_ROUTING_BUILD_BRIEF_v1.0`). Additionally, every build must (1) log telemetry to `Ops_Telemetry`, (2) declare its own success metric — no build ships without a way to measure whether it works, and (3) state what existing mechanism it reuses (reuse-first). This applies to every session, agent, and workflow.

**PAT-032: CONTINUOUS REVIEW — MANDATORY**
Every skill and process is reviewed on a cadence for effectiveness AND improvement by "The Optimizer" supervisory layer (see `CONTINUOUS_IMPROVEMENT_STRATEGY_v1.0`). The Optimizer runs on real telemetry, proposes a **ranked Top-10** of improvements/new-things (ranked by impact = time/effort saved or cost saved, effectiveness never ignored; each with payoff + effort + risk + next action), and researches new skills/tools that fit Brett's constraints. Brett picks which to build each round; **unchosen items go to a persistent Bench and are re-scored every round** — a #7 today can become #1 later (live list: `OPTIMIZER_ROUND_LOG.md`). The Optimizer proposes; Brett disposes (nothing self-executes past a human gate); any shipped change is measured after the fact to confirm the payoff. `brett-flow` session-close gains a step: log the job's telemetry + flag anything the Optimizer should review.

**PAT-033: BRANCH-FIRST + STAGING-VERIFY — MANDATORY for worker.js / index.html / vendor.html / wo.html**
Any change touching `worker.js`, `index.html`, `vendor.html`, or `wo.html` lands on a branch — never a direct push to `main`. Cloudflare auto-deploys that branch's `worker.js` to the separate `maintenance-hub-staging` Worker service (own Sheet, own dashboard vars, live at `https://maintenance-hub-staging.brett-2f8.workers.dev` — see CREDENTIALS_MAP.md "Staging sandbox"); the frontend pages point at it with `?api=staging` (persists across taps until `?api=prod` clears it — see `worker.js`'s `isStaging()` and the matching `WORKER` IIFE in each HTML file). `test-verified-builds` / smoke checks (curl `/health`, confirm `sheet_tail` is the STAGING tail not prod's, confirm no real SMS/QB/Gmail fired — staging stubs all three, see `env.__STAGING__`) run against that staging URL, never production. Only after staging verification passes does the branch merge into `main`, which is what actually triggers the production deploy. This exists because a non-prod branch build once deployed straight to production (the wrangler.toml July-21 incident) and because a Sept 1, 2026 session pushed straight to `main` and only tested against production after the fact — both are exactly the failure this pattern closes. Read-only doc/context-only changes are exempt (no blast radius); anything with a `worker.js`/frontend diff is not.
...
**Efficiency + improvement architecture (as of July 22):** AI cost is controlled by the **model router** (PAT-031 — cheap-by-default, escalate-on-need, direct-to-provider, telemetry-logged; see `MODEL_ROUTING_BUILD_BRIEF_v1.0`). System improvement is driven by **The Optimizer** (PAT-032 — a supervisory layer that watches telemetry, proposes ranked data-backed improvements + new lines, and researches new skills/tools that fit; see `CONTINUOUS_IMPROVEMENT_STRATEGY_v1.0`). We do NOT install third-party AI gateways (OmniRoute) or compression tools (Headroom) — reviewed and rejected for security/fit; ideas adapted into the owned router + brett-flow instead.
...
| Session 5 | July 22, 2026 | **Efficiency-as-a-system.** Re-reviewed Headroom (context-compression — its win we already get via brett-flow+CODEMAP) and OmniRoute (multi-model gateway — rejected: proxies business data through third parties). Chose the **secure DIY router**: call Gemini/Claude directly from our own Worker, cheap-by-default + escalate, telemetry-logged. Wrote `MODEL_ROUTING_BUILD_BRIEF_v1.0` (B-127). Then designed **"The Optimizer"** — a supervisory continuous-improvement layer (Instrument→Review→Research→Propose→Decide→Measure) that watches telemetry, brings ranked data-backed improvements + new business lines, and researches new skills/tools that fit; `CONTINUOUS_IMPROVEMENT_STRATEGY_v1.0` (B-128..B-132). Added **PAT-031** (route+instrument+measure+reuse by default) and **PAT-032** (continuous review). Worked reuse example: WO engine → Winchester driver payments → recruitment loop (B-132). |
| Session 6 | Sept 2, 2026 | **Staging deploy gate built.** Corrected a stale-doc assumption before building: CREDENTIALS_MAP's "Staging sandbox" note described the July-21 B-103 design (hostname-swap on `maintenance-hub` itself) — verified via `BUILD_ORDER_v1.0` + a live Cloudflare check that this was rejected July 23 for shared-preview-secrets risk and replaced by a genuinely separate Worker service, `maintenance-hub-staging` (already live, own Sheet). Added `isStaging(env, url)` to `worker.js`, stubbing QuickBooks writes (`qbApi`'s single chokepoint — covers every `/qb/*` write + internal QB-booking path with one change), Twilio SMS (`sendSMS`), and Gmail send (`gmailSendEmail`) whenever staging — reads stay live. Frontend `WORKER` swap (`?api=staging`, persists via localStorage, `?api=prod` resets) added to `index.html`/`vendor.html`/`wo.html` with a visible on-page staging banner. Added **PAT-033** (branch-first + staging-verify, mandatory for worker.js/index.html/vendor.html/wo.html). Corrected CREDENTIALS_MAP's stale staging section and `AUTONOMY_GUARDRAILS_v1.0`'s Rung-1 "tested" definition to point at the real staging URL. Drive-write stubbing explicitly deferred (not centralized like the other three — see FEATURE_LOG for the follow-up). |

---

*v1.13 — September 2, 2026 | Load at the start of every session*
