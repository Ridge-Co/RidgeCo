...
| Admin compose boxes — Entry_Notes + Hold_Reason (B-127) | 🟡 Built July 21 — staging | **WO detail panel:** ENTRY/ACCESS NOTES shows the property `Access_Notes` default above the per-WO thread + an append box → `/wo/append-entry-note`; HOLD REASON set/clear → `/wo/admin-update`. **Create form:** Entry/Access Notes + Hold Reason fields; `createWorkOrder` writes `hold_reason` directly and appends `entry_notes` attributed (append-only, so via `appendWOField`, never the row map). **Edit form:** Hold Reason is an editable field (replace); Entry_Notes is an "Add entry note" **append** box (a separate `/wo/append-entry-note` call after the bulk save) — never a bulk-overwrite field, per rule 19. Uses `loadAll()` refresh (rule 9). | Not verified live |
| Optional `ADMIN_TOKEN` tier | 🟡 Built July 21 — **INERT until the env var is set** | When `ADMIN_TOKEN` is set, bulk reads exposing notes/cross-record PII (`/workorders`, `/tenants`, `/owners`, `/keys`, `/master-keys`, `/keys-history`, `/smslog`, `/units`, `/properties`, `/vendors`, `/invoices`, `/materials`, `/returns`, `/owner-users`, `/wishlist`, `/wo-templates`, `/cache`) require it; the portal secret gets 403. Verified no customer portal calls any of them. No behaviour change while unset. See rule 23. | Not verified live |
| Pre-merge security review fixes | ✅ Fixed July 21 | Three independent reviewers over the full staging↔main diff. Fixed: stored XSS via `linkify` (rule 24, CRITICAL); private note text reaching the owner portal through the audit trail (rule 25, CRITICAL); `detectSource` substring domain match accepting `managebuilding.com.attacker.net`; SSRF + followed redirects in `ingestFiles`; chunked-response OOM bypass of the size guard; uncapped files per email; `notify_owner_status_note` promoting admin-only text to a customer-facing field; global (unscoped) tenant phone match. Pinned by `test/security.test.mjs` (66). | Offline tests only |
