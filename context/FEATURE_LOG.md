...
| Admin compose boxes — Entry_Notes + Hold_Reason (WO detail) | 🟡 Built July 21 — staging | ENTRY/ACCESS NOTES shows the property `Access_Notes` default above the per-WO thread + an append box → `/wo/append-entry-note`. HOLD REASON set/clear → `/wo/admin-update`, labelled "visible to owner + tenant". Uses the `loadAll()+openWODetail()` refresh (rule 9) and `insertAdjacentHTML` (rule 15). | Not verified live |
