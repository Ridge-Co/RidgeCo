...
18. **`enrichWO` starts from `{...wo}`, so EVERY new Work_Orders column is exposed to EVERY role by default.** Adding a column is therefore a visibility decision, not just a schema change: if it's private you must delete it in the `tenantView`/`ownerView` blocks (`PRIVATE_WO_FIELDS`) or it ships to the browser. This is why `Entry_Notes` needed explicit stripping, and it's how `ownerView` was already leaking `access_notes` (the property's standing access notes) to owners before B-104. Server-side only — never rely on the portal to hide a field. When B-104 pass 2 adds `Vendor_Thread`/`Admin_Notes`, they are already in `PRIVATE_WO_FIELDS`; keep them there. (July 21, 2026.)
19. **`Entry_Notes` and `Vendor_Thread` are APPEND-ONLY.** Multi-source by design — Brett's "lockbox is red" and Buildium's "call 24h ahead" must both survive on the same WO. Always go through `appendWOField()` (read → append `[ts — author]` line → write). A plain `updateWOFields({Entry_Notes: …})` silently destroys every earlier line. Vendor access display merges the property default **and** the WO note (`[Access_Notes, Entry_Notes]`), never one-or-the-other — a WO note must never hide the standing property default. (July 21, 2026.)
20. **`Work_Orders.Notes` is owner- AND tenant-facing today** — `owner.html` (~line 659) and `tenant.html` (~line 391) both render it verbatim. Never route private/internal content there. This is why intake's entry info moved to `Entry_Notes`. B-104 pass 2 repurposes `Notes` → `Admin_Notes` (admin-only), which is a visible behavior change for both portals and needs a UI decision, not just a column rename. (July 21, 2026.)
