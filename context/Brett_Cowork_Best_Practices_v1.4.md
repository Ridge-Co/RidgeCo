...
*Last updated: Aug 8 2026 v1.4 | Reference at the start of every session*

---

## ⭐ STANDING RULE — NO CLICKABLE QUESTION FORMS ON MOBILE (Aug 8, 2026)

**Never use the `AskUserQuestion` clickable-form / multiple-choice widget when Brett is on the
mobile app.** It hangs on mobile pretty much every time — to the point it blocks him from working.
Check the session `<env>` block: **`Client: mobile app` → do NOT call `AskUserQuestion`.** Instead
ask the SAME question in **plain text with lettered options (A/B/C)** he can reply to by letter — he
still wants multiple-choice, just delivered as text, not the widget. On **desktop / laptop** the
widget is great — keep using it there. When in doubt about the client, default to plain-text
lettered options. This is the MO for all Cowork chats going forward, per Brett's explicit request.
...
Per PAT-012: ask for lettered options. If the AI asks something open-ended, redirect: "Give me 3 options, lettered A/B/C." **Delivery matters (see the ⭐ standing rule at the top): on mobile, lettered options go in PLAIN TEXT, never the `AskUserQuestion` clickable widget (it hangs on mobile). Widget is fine on desktop/laptop.**
