...
| AUTONOMY_RUNG2_SAFE_CLASS_BUILD_BRIEF_v1.0.md | v1.0 | ⏳ ON-DEMAND (Rung-2 auto-ship build) | **B-239.** Execution contract for turning on Rung-2 auto-ship for the SAFE class only (Brett's Sep 17 decisions: SAFE-class-only scope, per-item not whole-batch hold-on-smoke-fail). Recommends a new Worker cron ("Optimizer Ship") rather than a headless Cowork session, reusing B-129's proven secret-backed pattern instead of GH Broker's unconfirmed headless reachability. `Ops_Build_Queue.Risk_Class` shipped same day; staging deploy gate merge, smoke-test harness (B-141), a Worker-reachable GitHub push path, and rollback (B-148) are still needed — none built yet. |
