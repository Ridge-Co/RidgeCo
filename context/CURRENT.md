...
## 🟢 Live: Ridge Co–supplied materials on scope proposals + Reconnect Gmail — PR #22 merged Sep 22 (`eaa583b`). Full detail FEATURE_LOG [FL-20260922-1500-rm]
**Status (Sep 22, 3:42pm ET):** Brett added the `/gmail/callback` redirect URI, merged, and reconnected Gmail via the new button — `/health` confirms `gmail.config_token_set:true`, `config_token_updated 2026-09-22T19:35:15Z` (token now lives in Config; consent screen is In production, so no 7-day expiry). All Hub email is back (estimates, vendor invoice confirmations, QB receipt forwarding, AR weekly report, selftest digest, property notices); receipts that failed to forward since ~Sep 14 should catch up on the next daily forwarding run — worth a glance. Brett re-priced the $1-hack scope to use RC materials. Next time Gmail breaks: Scope Creator → Send → 🔑 Reconnect Gmail (no Cloudflare secret edit needed); `GET /gmail/token-check` (admin) is the no-send check.
**Open follow-ups (not built):** materials overage has no in-Hub billing path (bill manually in QB; belongs with the change-order system); vendor-portal bill receipts (`Receipts_Total`) on a proposal job aren't covered by the double-billing guard. `BUILD_VERSION` wasn't bumped (kept PR #21's value to avoid a conflict) — this build is identified by `/health` `gmail.config_token_set` being present.
**Original PR notes:** Scope Creator options get a green "RC mat. $" + description (materials Ridge Co buys — never on the vendor bill/milestone Vendor_Amount; materials-only items no longer need a fake $1 vendor cost). Per-proposal pricing: same markup / at cost + card fee / flat % + card fee. Owner sees "Includes materials: desc ($X)" only. Signed Proposals shows materials budget vs matched receipts; receipts on a materials-priced signed proposal's WO are blocked from Review Bills (proposals without materials unaffected). Gmail: the send failure was the OAuth app in Testing (7-day token expiry) — **Brett published it to In production Sep 22**; the old token is dead and was NOT re-minted by hand. New "🔑 Reconnect Gmail" (Scope Creator, shows after a Gmail send failure) signs ridgecomaintenance@gmail.com in and stores the token in Config.
**✅ Done by Brett Sep 22 (kept for reference):** (1) Google Cloud → Clients → OAuth client (must be Web application) → add redirect URI `https://maintenance-hub.brett-2f8.workers.dev/gmail/callback`; (2) merge the PR; (3) Scope Creator → Send → Reconnect Gmail → sign in as ridgecomaintenance@gmail.com → Send again. Until then every Hub email is down (estimates, vendor invoice confirmations, QB receipt forwarding, AR weekly report, selftest digest, property-notice emails). (4) Re-price the scope that used the $1 hack: set that item's vendor cost blank and put the amount in RC materials, regenerate.
...
Pushed as a branch + PR (`feature/vendor-performance-dashboard`, PR #19), not direct to `main`
— per PAT-033, and per this file's own repeated flags this session about CAP-035/Queue #24
skipping the branch-first+staging-verify step. Brett reviewed and merged PR #19 himself.

**Live-verified after merge, all 6 acceptance criteria from the brief:** `/health`'s
`build_version` confirmed the Cloudflare deploy actually landed (`2026-09-22.1-vendor-performance-dashboard`).
`GET /vendor-performance` with the admin token → 200, 16 active vendors, $22,996.32 total
billed, 30 open jobs; same call with no token → 401. Read-back check on vendor_id 5 (Eddie
Smith): `jobs_total`/`total_billed`/`bill_count` all matched `/vendor-workorders` and
`Vendor_Bills` exactly (18 / $7,158.00 / 14). UI check done via a headless Chromium in the build
sandbox (Chrome extension wasn't connected) at a real phone viewport: real login, zero console
messages/errors, sort-by-Billed verified strictly descending by extracting the actual DOM cell
values (not just eyeballing a screenshot — the Billed column scrolls off-screen at phone width,
which nearly produced a false read), sort-by-Vendor verified alphabetical. Zero-writes confirmed
both by code (the function contains only `fetchTabs`+`json`, no write call exists) and live
(`/health`'s tab row counts identical before/after every test run).

**Real finding surfaced by the live data, not a bug**: Andreas Cleaning Service shows $3,180.97
billed against 0 currently-linked work orders — bills on file with no attached WO. Worth a look
given the active plan to cut them as a vendor (still-unpaid-by-customer jobs, repriced to a 5%
processing fee).
...
| CONTINUOUS_IMPROVEMENT_STRATEGY_v1.0.md + **v1.1.md** | v1.1 | ⏳ ON-DEMAND (strategy/optimizer) | "The Optimizer" — supervisory continuous-improvement layer (Instrument→Review→Research→Propose→Decide→Measure). Watches telemetry, proposes a ranked **Top-10** by impact + a carry-forward **Bench**, researches fitting new skills/tools. Governs per PAT-032. B-128..B-132, **B-238**. **v1.1 (Sep 17): 4th lens — Product/UX Opportunity, folded into the existing Scout & Reuse-Radar Mon/Thu task — plus a standing queue-integrity self-check on `Ops_Build_Queue` (auto-drop clearly-superseded greenlit items with a logged reason, flag ambiguous ones for Brett).** |
...
| AUTONOMY_RUNG2_SAFE_CLASS_BUILD_BRIEF_v1.0.md | v1.0 | ⏳ ON-DEMAND (Rung-2 auto-ship build) | **B-239.** Execution contract for turning on Rung-2 auto-ship for the SAFE class only (Brett's Sep 17 decisions: SAFE-class-only scope, per-item not whole-batch hold-on-smoke-fail). Recommends a new Worker cron ("Optimizer Ship") rather than a headless Cowork session, reusing B-129's proven secret-backed pattern instead of GH Broker's unconfirmed headless reachability. `Ops_Build_Queue.Risk_Class` shipped Sep 17; the Prepare-agent write path (greenlit→prepared bridge) shipped Sep 18 as **B-240**; the smoke-test harness (**B-141**) was already done back on Aug 22 (a prior version of this row wrongly listed it as missing). Still needed: staging deploy gate merge, a Worker-reachable GitHub push path, and rollback (B-148). |
...
## 🟡 Open PR (not yet merged): PR #14 (`optimizer-v1-2-scout-queue-write`) — Optimizer v1.2, Scout & Reuse-Radar backlog-writing
Appended here rather than at the top of this log because the top section above already reflects everything currently live on `main`; this PR's own entry (verbatim, as written on its branch) follows so nothing from it is lost pending merge:

# WHERE THINGS STAND — Sep 20, 2026 (Optimizer v1.2 — Scout & Reuse-Radar becomes backlog-writing, competitor-benchmarking, 20-item)

## 🟡 Built, PR #14 open, not yet merged/live: Scout & Reuse-Radar gets a real write path into Ops_Build_Queue — full detail FEATURE_LOG rule 196, `context/CONTINUOUS_IMPROVEMENT_STRATEGY_v1.2.md`
New `POST /ops-queue/scout-submit` (narrow `SCOUT_QUEUE_TOKEN`, insert-only, `Status` hardcoded to `'proposed'` server-side) lets the twice-weekly Scout & Reuse-Radar task land its ranked findings as real `Ops_Build_Queue` rows instead of only a chat message. New `'proposed'` status ahead of `greenlit` in the lifecycle; new columns `Lens`/`Requires_Spend`/`Spend_Note`/`Workaround`; `opsQueueUpdate` gains an optional `risk_class` field so `proposals.html`'s new "Approve → Greenlit" action can set it at promotion time. `proposals.html` gained a "Proposed — from Scout & Reuse-Radar (needs your review)" section (Approve→Greenlit / Drop). `test/ops-queue-scout-submit.test.mjs` 57/57; full suite 88 files/87 pass/1 pre-existing unrelated failure (`trade-map.test.mjs`, same on unmodified `main`). Classified SAFE per Brett's own Sep 20 sign-off — insert-only, structurally can't reach `greenlit`/`building`/`done`, no money/PII/auth — recorded in the v1.2 addendum since it didn't cleanly fit the SAFE list's literal "new isolated tab only" wording.

**Needs Brett:** (1) set the `SCOUT_QUEUE_TOKEN` Cloudflare Worker secret; (2) merge PR #14 (`optimizer-v1-2-scout-queue-write` → `main`). Neither has happened yet — this is built and tested but not live.

## 🟢 Already live (no merge needed): Scout & Reuse-Radar trigger prompt rewritten — 4th lens actually wired in, named competitors, 20-item cap, softer shape-filter
Separately, and already in effect: the Scout & Reuse-Radar Cowork task's own stored trigger prompt was rewritten directly via its scheduling API this same session (not a repo file, no PR needed). Found and fixed a real gap first — v1.1's Product/UX lens + queue-integrity check (B-238, Sep 17) had never actually made it into the live prompt; the trigger still hardcoded its old 3-lens list. Now: 4 lenses (Outward incl. named competitors — AppFolio, Buildium, Rentvine, Propertyware, Yardi Breeze, TenantCloud, Jobber, Housecall Pro, ServiceTitan; Inward/Reuse-Radar; Product/UX; Queue Integrity), cap raised "up to 10" → "up to 20" on both Mon/Thu runs, new `FUTURE DIRECTION` tag for resurfacing deliberately-deferred features with competitive context, and the old hard architecture-fit filter loosened to a preference (propose and adapt rather than silently discard) while keeping a real, now-structural spend-discipline filter (`requires_spend`/`spend_note` + suggested workaround). Step 6 now calls the new `/ops-queue/scout-submit` endpoint above — dormant until Brett sets `SCOUT_QUEUE_TOKEN` and merges PR #14.
