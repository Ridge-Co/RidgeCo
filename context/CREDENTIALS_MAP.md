...
**`CONTACTS_SYNC_TOKEN` (added Aug 6, 2026):** dedicated **read-only** secret for the Google Contacts sync (Apps Script under brett@bmoremanagement.com). Worker accepts it for **GET only** on `/tenants`, `/owners`, `/vendors`, `/properties`, `/units` — never writes, never any other path; fully inert unless set (worker.js auth gate ~L49). Independent of `WORKER_SECRET` — rotate/revoke this one alone without affecting portals or admin. Same value is stored in the Apps Script Script Property `WORKER_SECRET`. Auth-boundary smoke-tested live Aug 6 (5 reads 200; writes + other paths + no-token all 401). **Do NOT set `STAGING=1`** as a global var — it would flip production into staging mode (reads `STAGING_SHEET_ID`, breaks the live Hub). Staging is auto-detected from the `staging-` preview hostname instead (worker.js ~L45).
