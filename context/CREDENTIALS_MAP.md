...
**Version:** v1.4 | **Last Updated:** September 2, 2026
...
**Known worker vars/secrets (verified July 21, 2026, corrected Sept 2026 — see Staging sandbox note below):** `SHEET_ID`, `GOOGLE_SA_EMAIL`, `GOOGLE_SA_KEY`, `DRIVE_PROPERTIES_ROOT`, `DRIVE_VENDORS_ROOT`, `KEY_REGISTRY_SHEET_ID`, `WORKER_SECRET`, `INTAKE_TOKEN`, `TWILIO_SID`, `TWILIO_AUTH`, `TWILIO_FROM`, `QB_CLIENT_ID`, `QB_CLIENT_SECRET`, `QB_REALM_ID`, `QB_REFRESH_TOKEN`, `CONTACTS_SYNC_TOKEN`, `STAGING` (see below — `maintenance-hub-staging` only, never on `maintenance-hub`).
**`CONTACTS_SYNC_TOKEN` (added Aug 6, 2026):** dedicated narrow-scope secret for the Google Contacts sync (Apps Script under brett@bmoremanagement.com). Worker accepts it for **exactly two things** (auth gate ~L49): (1) **GET** on `/tenants`, `/owners`, `/vendors`, `/properties`, `/units`, and (2) **POST `/contact/augment`** — an augment-ONLY write that fills blank fields from an allow-list (`Email` only), never overwrites, never touches phone/ID, never creates rows, logs to `Contact_Augment_Log`. Nothing else — every other path/method 401s. Fully inert unless the env var is set. Independent of `WORKER_SECRET` — rotate/revoke alone without affecting portals or admin. Same value stored in the Apps Script Script Property `WORKER_SECRET`. Auth-boundary + augment guarantees smoke-tested live Aug 6 (reads 200; other writes/paths/no-token 401; overwrite refused with read-back; Phone/ID rejected from augment).
**Staging sandbox — CORRECTED Sept 2026 (staging deploy gate build).** The B-103 design referenced here until now (a `staging-` hostname prefix on `maintenance-hub` itself, swapping `SHEET_ID`→`STAGING_SHEET_ID`) was never actually built — `BUILD_ORDER_v1.0` Phase 1.1 rejected it July 23 (preview/branch deployments of the SAME Worker share that Worker's secrets globally per the line above, so a preview would read/write the real Sheet) and shipped the real fix instead: **`maintenance-hub-staging` is a separate Cloudflare Worker *service*** (its own entry in Workers & Pages, own independent Settings → Variables and secrets — NOT a preview of `maintenance-hub`), Git-connected to the `staging` branch, live at `https://maintenance-hub-staging.brett-2f8.workers.dev`. Its own `SHEET_ID` var already points directly at the isolated staging sheet (`16PCD3tIDatZLhMeHdbeYC-4R4BVNCcZ26iBY90H6dFY`, tail `0H6dFY`, shared with the runtime SA above) — **no hostname-swap code needed or built for this**, and `STAGING_SHEET_ID` was never read by any code; it's a dead var name, safe to remove if still present anywhere. What Sept 2026's build added: `isStaging(env, url)` in `worker.js` (~L35), checked once per request and stashed on `env.__STAGING__`, stubs QuickBooks writes/Twilio SMS/Gmail send (reads stay live) so staging can't move real money or contact a real customer regardless of what its own QB/Twilio/Gmail secrets are set to. Detected via `env.STAGING === '1'` OR the request hostname containing "staging" (true today with zero setup). **`STAGING=1` is safe to add as a Var ONLY on the `maintenance-hub-staging` service** — because it's a genuinely separate service now (not a shared-secret preview of prod), setting it there cannot affect `maintenance-hub`. Never add it to `maintenance-hub`'s own Settings.
