...
- B-233 · 🟠 **Combine/merge work orders without losing data** (Brett, Aug 24). Select 2+ WOs → combine into one, with a reason (duplicate / convenience / same-visit) logged. The absorbed WO's full history — billing/materials, photos, tenant comms/updates — must survive on the surviving (parent) WO, not be dropped. If the absorbed WO had already notified its tenant (status progressed past intake), that tenant gets a follow-up notification the two were combined, with the reason and a portal link to the surviving WO; a WO with no tenant comms yet is folded in silently. Precedent: `adminMergeOwner`/`adminMergeProperty` (worker.js ~3918/4038) already show the shape for this class of merge on Ridge Co — preview-first, re-point every reference table, deactivate-not-delete the loser, carry over anything the keeper is missing. **GATED-adjacent, build-with-Brett** per `AUTONOMY_GUARDRAILS_v1.0` (touches tenant comms + billing history) — same posture as B-211. **Open design questions before building** (asked Brett directly, Aug 24): which of the selected WOs becomes the parent when it's not obvious; whether combining ever needs to span two different tenants/properties or is always same-tenant/same-property; and whether a combined-notice SMS is sent per-tenant-thread or Brett reviews/edits it before send like other tenant-facing messages.
