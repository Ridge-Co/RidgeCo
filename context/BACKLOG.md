...
**IN PROGRESS**
- B-224 · 🟠 **Invoice from materials/time — remove the vendor-bill gate** (Aug 12; from the WO-1099 review). The WO billing panel won't let an invoice START unless a `Vendor_Bills` row exists ("No vendor bill to invoice yet"), which blocks the "Brett did the job" case entirely and makes "vendor, our-account materials" clunky. Fix: build the invoice from ticked materials + logged time with **no vendor-bill gate**; **materials always on the customer invoice with markup**; a **three-way job-type picker** (Brett-did-it / vendor-our-account / vendor-own-materials) drives whether & what vendor bill is created; per-material **Paid by: Our account / Vendor** toggle (replaces the confusing "untick if already on vendor's bill" caption). Backend already splits receipts correctly (`pay=account` → customer invoice only; else also vendor bill) — the gate is the blocker. Money/QB-facing → preview-first, held for Brett. **Full spec + markup math: PRIVATE `brett332/data` → `business-context/billing-model.md` (keep out of this public repo).**
