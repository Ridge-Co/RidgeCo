...
- B-214 · 🟠 **Overnight Rung-1 Prepare agent (the "improve while I sleep" engine).** Scheduled task that takes the top greenlit item, drafts the full change + runs `test-verified-builds` + `ridgeco-validate`, and leaves it ready for Brett's one-tap morning approval — **never deploys** (AUTONOMY_GUARDRAILS Rung 1). Unblocked now the guardrail is locked; needs B-210 (delivery) to surface output. This is the core of Brett's Aug-7 autonomy ask.
