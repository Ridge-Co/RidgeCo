...
**Version:** v1.40 | **Last Updated:** Aug 24, 2026
...
**IN PROGRESS**
- B-224 · ✅ **Invoice from materials/time — vendor-bill gate removed** (Aug 12, SHIPPED — FEATURE_LOG rule 83; `test/invoice-no-bill.test.mjs`, 13 assertions). The WO billing panel no longer requires a `Vendor_Bills` row to START an invoice: `invPricing` prices from ticked materials + logged time against a synthetic $0 bill, `approveInvoiceReview` accepts a blank `bill_id`, and the send (already $0-vendor-cost aware) posts the customer invoice only — materials counted ONCE (own-materials), never doubled. The three entry methods (bill hours / hand bill / portal) are unchanged; only the block is gone. Per-material **Paid by** already exists on the hand-bill form (`pay` account/reimburse). Send is still preview-first + Brett-gated. **REMAINING: Brett's one supervised live send** — a WO with materials + no vendor bill → Approve & send → confirm QB posts the invoice and NO vendor bill. **Spec + markup math: PRIVATE `billing-model.md`.**
- B-225 · 🟢 **Admin/coordination time capture + auto-time capture** (Aug 12, roadmap). (a) An admin/coordination charge for the human-equivalent effort of running a job (folded into the invoice total, **not** a visible line item, **not** marked up). (b) Auto-capture Brett's on-site labor AND admin time by tying together **calendar + SMS/Twilio + Google Maps** location history (materials-pickup trips, site visits, per-job texting/calling) so his time stops going unbilled. Pricing principle it serves: **charge to complexity + the friction the customer/tenant causes**. **Details + figures: PRIVATE `billing-model.md`.**
...
- B-205 · ✅ **Trash Service — one-tap recurring flat-rate billing** (shipped Aug 7, `2026-08-07.6`). `trash.html` + 9 `/trash/*` Worker endpoints + `Trash_Properties`/`Trash_Visits` tabs. Tap a property → before/after photos → optional $20-increment extra → Save (logs the visit); one weekly QB invoice/property, preview-first, photos attached; in-app "Needs attention" nudge live. **REMAINING:** (1) Brett's first real invoice send off a preview (never run against live QB); (2) phone-push nudge — set Cloudflare env `TRASH_NUDGE_TOKEN` + create the scheduled task (endpoint gate already deployed, inert). See FEATURE_LOG rules 50–53. (Commit labeled "B-203" in error — B-203 is the tenant-transfer item below.) **Aug 22 additions (FEATURE_LOG rules 132–133, `2026-08-22.1`, 🔴 none Brett-verified yet):** "Add photos" precise-tap-only bug fixed (was a `<label>` wrapping a hidden file input); selectable service date on Log Visit (defaults to today); This week/Last week tabs (backend already supported `?week=`, only the UI never exposed it); batch "select multiple → Preview & send all" (loops the existing preview-first per-property send, no new money-path engine); "Nothing done" mark-skipped per property/week (new `Trash_Skips` tab, suppresses the missed-visit nag without deleting the record — distinguishes "consciously skipped" from "forgot").
...
- B-227 · ✅ SHIPPED Aug 22 (FEATURE_LOG rule 134, `2026-08-22.2`) — Repo-wide sweep of the label-wraps-hidden-file-input tap-target bug. Fixed all 12 remaining sites across 6 files: `wo.html` (Before/After/Problem + receipt photo, 4 spots), `scope-creator.html` (handwriting upload, before-photos), the shared WO-photo-upload button builder used by `index.html`/`tenant.html`/`owner.html`/`vendor.html` (1 fix, 4 files), and vendor.html's own invoice-file + per-row receipt-photo buttons (2 spots). Also bumped touch-target size (min-height) and inter-button gap on the affected rows, and on `scope-creator.html`'s `.btn`/`.btn.small` generally, per Brett's broader "easy to use, not exact precision, no accidental double-press" ask. 🔴 None Brett-verified yet.
- B-228 · ✅ Answered Aug 22 — Brett meant a build-time QA checklist ("we are building something... don't want to encounter that bug again"), not ClickUp or the Hub Wishlist. Written as `context/UI_QA_CHECKLIST.md`, referenced as a new ⚡ standing rule at the top of CURRENT.md — covers full-tap-box pattern, min touch-target size, adjacent-button spacing, and double-submit protection (points at vendor.html's existing `claimSubmit()` guard as the pattern to reuse).
- B-229 · ✅ SHIPPED Aug 22 (FEATURE_LOG rule 131, `2026-08-22.1`) — Voice-to-text cut off a few seconds into any pause in speech (rule 102 mic, all 4 portal/Hub files). No cross-browser API exposes the silence-timeout duration Brett wanted turned up, so fixed the actual behavior instead: `rec.onend` auto-restarts the recognizer unless the user explicitly tapped stop or hit a fatal permission error — dictation now survives a pause of any length rather than a fixed few extra seconds. 🔴 Needs Brett's first live pass.
- B-232 · ✅ Answered Aug 24 — QB Matching page confusion, no code bug found. Investigated Brett's 4 reports against the live code + his own screenshots (`qbEntities`/`qbSetMap`/`qbMapEntity`, worker.js ~7120-7290, ~8130-8330): (1) **Phoenix Estates / Goldszmidt Properties "Link doesn't stick"** — both are DUPLICATE Owner rows (screenshots show a second "Phoenix Estates" and "Goldszmidt Properties" row sitting beside an already-linked one). `qbSetMap`'s `shared_conflict` guard (worker.js `qbMappingClash`) is working as designed — it refuses a second Hub owner pointing at a QB id already claimed by another active owner, and its confirm() dialog explicitly says "cancel and use Find duplicate owners instead." Real fix is `adminMergeOwner` via the existing "🔎 Find duplicate owners" Dev-tools button (`checkDuplicateOwners()`, line ~593) — NOT the Link button — which carries the QB id over to the keeper automatically. Not a bug; Brett needs to run the merge tool on these two names. (2) **2725 Fait Ave linked directly to owner #342, not as a nested sub-customer** — confirmed correct per his hypothesis (Andrew Avramidis has only this one property, and it's mapped straight to his top-level QB customer rather than a "Andrew Avramidis:2725 Fait Ave" sub-customer). Billing still works either way; for consistency with every other multi-property owner, recommend clearing that property's link and using "+Create" to make a proper nested sub-customer — low urgency, optional. (3) **931 Saint Paul St units missing from the dropdown** — the PROPERTY itself was never created in QuickBooks (screenshot shows "+Create" on the property row, "link its property first" on all 6 unit rows below it) — expected behavior, not a bug: `qbCreateSubCustomer` requires the parent to exist first. Brett's earlier bill for that address almost certainly posted to the owner's top-level customer, not a sub-customer, which is why it's invisible as a "sub-customer of the owner" in QuickBooks. Fix: tap **+Create** on the "931 Saint Paul St" property row, then each unit's own +Create becomes available. (4) **3014 N Calvert St Apt B (basement) — "no linked property" despite #422 showing linked** (raised mid-session, not yet visually confirmed) — likely one of: the basement apartment was entered as its own standalone Property record instead of a Unit nested under the already-linked 3014 N Calvert property (would show as an unlinked PROPERTY row, not a unit); or the Unit was never created in the Hub's own Units tab for that address at all (nothing to link). Gave Brett a 3-step checklist to confirm which on the matching page; needs his screenshot of the PROPERTIES/UNITS sections for that address to close out. **Not logged as a code defect — reopen as a real bug only if the checklist rules out both explanations.**
  **Correction, same day (Aug 24, later):** item (1) above was wrong about the actual cause. Brett ran "Find duplicate owners" for real and both "merge it away" and "make a second login" refused to do anything, reappearing every check. Root cause, confirmed by reading the live Owners tab directly (Google Drive connector, not guessed): these are NOT duplicate records — **both pairs are two real, different people sharing one Owner ID number**, not two rows for one person. Phoenix Estates: Amanda Ireland (PIN PHX58842, QB #329) and Mark Passerelli (PIN PHX64174) were BOTH stamped ID 1. Goldszmidt Properties: Jennifer Goldszmidt (PIN JEN45614, QB #297) and Adrian Goldszmidt (PIN ADR95237) were BOTH stamped ID 3. `adminMergeOwner` and `adminOwnerToUser` both correctly refuse `from_id === to_id` (`"Both rows carry the SAME ID... give one of them a unique ID first"`) — that refusal, not a UI failure, is why nothing appeared to happen. **Fixed live** via the `context/sheet-ops` pipeline (`set_cell` ops, not `update_cell_by_id` — that matches by ID and would have hit the wrong row given the collision): Owners!A3 (Mark) → 18, Owners!A5 (Adrian) → 19, verified by re-reading the sheet after the Action ran. Nothing else on either row touched. **Next step for Brett:** re-run "Find duplicate owners" — Phoenix/Goldszmidt should now show as two genuinely distinct IDs, and the "Make ID 18 a second login on ID 1" / "Make ID 19 a second login on ID 3" buttons should now actually work (creates each person's Owner_Users login on the keeper, carries their PIN + the QB link, deactivates nothing that matters). **Open question for a future session:** how did two rows get the same manually-typed ID in the first place — worth a quick look at whatever tool/import added the second contact, since this could recur for any other "second person, same company" entry made the same way.
- B-233 · 🟠 **Combine/merge work orders without losing data** (Brett, Aug 24; Q&A closed same day, ready to scope a build). Select 2+ WOs → combine into one, with a reason (duplicate / convenience / same-visit / misattributed-unit) logged. The absorbed WO's full history — billing/materials, photos, tenant comms/updates — must survive on the surviving (parent) WO, not be dropped. Precedent: `adminMergeOwner`/`adminMergeProperty` (worker.js ~3918/4038) already show the shape for this class of merge on Ridge Co — preview-first, re-point every reference table, deactivate-not-delete the loser, carry over anything the keeper is missing.
  **Answered Aug 24:**
  1. **Parent pick = manual.** Brett ticks the WOs to combine, then explicitly picks which one survives (matters most on the "tenant already notified on WO #1" case).
  2. **Combine CAN span tenants/units within the same property** — real example given: a leak in 2F turns out to actually be sourced from 3F, so the 3F WO gets folded into the 2F WO it was mis-created against. Property is assumed to stay the same; **tenant/unit does not** — data model needs a combined WO to carry a LIST of tenant/unit refs (not the current presumed one-tenant-per-WO shape), and the merge plan must show every unit/tenant losing its dedicated WO_ID reference across whatever tables key on it (mirrors the `OWNER_REFS`/`PROPERTY_REFS` re-point pattern, needs a `WO_REFS` equivalent).
  3. **Tenant notification = preview-then-send, opt-IN per tenant, default unchecked.** Combine screen must show one checkbox per affected tenant thread ("notify this tenant their WO was combined, with reason + portal link") with **every box defaulting to unticked ("cancel outgoing")** — nothing texts a tenant unless Brett actively ticks it and confirms. No auto-send path.
  **GATED-adjacent, build-with-Brett** per `AUTONOMY_GUARDRAILS_v1.0` (touches tenant comms + billing history) — same posture as B-211. **Next step:** a build session to (a) confirm the current Work_Orders/tenant/unit reference shape (is it already 1 tenant per WO everywhere, or does multi-unit already exist somewhere), (b) design the `WO_REFS` merge-plan table, (c) build the combine UI (multi-select on the WO list + reason + per-tenant notify checkboxes) mirroring `adminMergeOwner`'s preview-first pattern, (d) wire the opt-in notification send through the existing `queueNotification`/tenant-portal-link machinery.
- B-234 · ✅ SHIPPED Aug 24 — Bulk access-info tools, from Brett's "bulk update section... data point #1 (every Goldszmidt unit on their master key), data point #2 (no per-unit lockboxes, buildings work off a front door and/or service door code)." Three pieces:
  1. **Data point #1 already had a tool — Brett wasn't blocked.** The Master Keys panel (Hub → Shopping/Materials → Master Keys tab → "+ Add Master Key" → "Assign to Owner") already bulk-writes `Properties.Master_Key_ID` across every property of a chosen owner (`bulkAssignMasterKey`, shipped pre-existing). What was actually missing: the assigned master key was written but never READ anywhere downstream, so a vendor dispatched to a master-keyed property got zero access info. Closed that gap — `getWOLockboxes` (worker.js ~711) now accepts a resolved `masterKeyRow` and synthesizes a "Master Key — {Name}: {Code}" entry into the same list a lockbox would appear in, so it now reaches BOTH the vendor dispatch SMS (`assignVendor`) and every portal view (`enrichWO`, all 4 call sites + the shared WO link). Also added a real **Code** field to `Master_Keys` (was Name/Owner/Notes only — no usable key-cut number) plus an Edit action in the panel.
  2. **Lockbox Contents field — finished** (backend groundwork from earlier this session, now wired end-to-end). A lockbox on a multi-unit property no longer implicitly means "opens everything" — `Contents` (Front Door Only / Front Door + Unit Key / Unit Key Only) is captured on Add/Edit Access Item, shown in the label everywhere `getWOLockboxes` output reaches (dispatch SMS, portal views), and flagged red ("⚠ contents not specified") on the property Keys admin view and the WO-detail live access-codes widget when a lockbox has none set — so the gap is visible, not silently assumed.
  3. **New: "📋 Bulk Set Building Access" tool** (Keys/Lockboxes tab) — data point #2, genuinely new. Pick an owner → every active property loads as an editable row (Front Door type/code/contents, optional Service Door same) → Save writes/updates one building-level Keys row per door per property in a single pass. Pre-fills from whatever Building-FrontDoor* or a Location="Service Door" custom item already exists, so re-running it later to fix a code UPDATES that row instead of creating a duplicate (matched on `Key_Type` for front door, `Key_Type` + `Location` for service door — see code comment on why that string is load-bearing). Unchecking a previously-set Service Door deactivates it rather than leaving a stale code floating.
  All three: `node --check` clean on worker.js and all 5 index.html inline script blocks; `test/qb-match.test.mjs` still 10/10 (untouched code path). 🔴 None Brett-verified yet — needs one real dispatch on a master-keyed property to confirm the SMS text, and one pass through the new Bulk Set Building Access tool on a real owner (Goldszmidt is the natural first test given his message).
...
- B-127 · 🟢 **Built Aug 22, first live call site wired Aug 23 (receipt_parse via `receiptExtract`) — see BRETTOS INFRASTRUCTURE table below for current detail.** Model-routing layer — secure DIY multi-model router in Worker (cheap-by-default + escalate; Gemini+Claude direct; PAT-031). Brief: `MODEL_ROUTING_BUILD_BRIEF_v1.0`. `routeAI` reuses `logTelemetry` (B-128). Real telemetry now flows on every receipt scan.
- B-128 · 🔧 BUILT + DEPLOYED (Aug 7, pending Brett's authed verify) · Job telemetry spine — `Ops_Telemetry` tab (self-provisions via `ensureTab`) + `logTelemetry` chokepoint + secret-gated `POST /telemetry/log` for Cowork/skill rows (dual-source: Worker + Cowork). Wired into `GET /daily-digest` (best-effort). Prerequisite for the whole Optimizer. **Brief: `TELEMETRY_SPINE_BUILD_BRIEF_v1.0`.** Deployed on `2026-08-07.4`. Adversarial review passed SHIP-WITH-FIXES → all fixes applied (ensureColumns + landed-guard, renamed to `logTelemetry`). H2 (Optimizer stuck-pattern Review step) + H1 (verifier rule, FEATURE_LOG 51) landed. **Remaining:** (1) Brett runs the one authed `POST /telemetry/log` smoke to confirm a row lands (needs WORKER_SECRET — Claude can't); (2) package the H1 rule into the `test-verified-builds`/`ridgeco-validate` skill files; (3) wire `brett-flow` session-close to POST one row per session; (4) B-127 `routeAI` reuses `logTelemetry`.
- B-129 · 🔧 BUILT + DEPLOYED (Aug 7) · Optimizer weekly Reviewer — **Worker cron** (`0 12 * * 1`, Mon 8am ET), NOT a Cowork task (headless sessions lack WORKER_SECRET/PAT — FEATURE_LOG 52). `runWeeklyReview`: 7d Ops_Telemetry → metrics + stuck-pattern flags (H2) → `claude-sonnet-4-6` ranked proposal (best-effort, skipped <5 rows) → `Ops_Review_Log` tab → delivers only if `digest_enabled=TRUE` (dormant: email stub, SMS down). On-demand: `POST /ops-review` + Hub Dev Log button "🔎 Run Ops Review"; `GET /ops-telemetry` reads the tab. Deployed `2026-08-07.5`. Adversarial review SHIP-WITH-FIXES → applied (POST not GET, manual never delivers, strict date filter, real Delivered status). **Next:** B-130 Scout (needs headless-auth or B-127) · richer proposals once B-127 routes the model call · a real email/SMS channel to push the weekly review instead of tab-only.
- B-130 · Optimizer: Scout agent — research new skills/tools/models, filter for stack-fit, propose
- B-131 · 🔧 LIVE (as a lens) · Optimizer: Reuse-Radar — spot shared mechanisms + combine-existing-tools opportunities. Now runs as the INWARD lens of the **Optimizer Scout & Reuse-Radar** scheduled agent (Cowork task `trig_01CphCdHS4...`, Mon/Thu 8am ET), alongside the OUTWARD Scout lens. First exemplar = B-204 (calendar invite on WO scheduling). Standing question: "we already do X and already have tool Y — what if X used Y?"
...
- B-204 · 🟠 **Calendar invite on WO scheduling (flagship Reuse-Radar find, B-131 exemplar).** When a WO is scheduled (`scheduleWO`, POST /schedule ≈2526), ALSO create a Google Calendar event (MCP already connected as info@bmoremanagement.com) and invite the right guests: the **tenant** (should be there), the **access-holder** (master-key holder or a flexible vendor like Alex Busey when relevant), and the property **OWNER for AWARENESS by default** — shared so they're informed, NOT "please attend" unless their presence is actually needed (case-by-case; default is inform-only). Updating the event (e.g., when Home Depot confirms the delivery window the day before) auto-notifies all guests; each can add to their own calendar + opt into reminders. Augments — does not replace — the existing tenant SMS with a live, self-updating entry. Guest-selection logic is per-case (who has access / who's flexible / does the owner need to attend or just know). Ties B-131 (Reuse-Radar), B-156 (Wave-0 tenant comms / scheduleWO), B-138 (contact channel map), Google Calendar connector. Origin: Brett's Aug-7 fridge+washer two-apartment scheduling (owner had master key + Alex flexible). The PATTERN — "we already do X and already have tool Y; what if X used Y?" — is now a standing lens in the Optimizer Scout+Reuse-Radar scheduled agent.
**OPTIMIZER ROUND 1 (Aug 7) — new items from the first Scout+Reuse-Radar scan (see OPTIMIZER_ROUND_LOG Round 1):**
> Renumbered B-205→B-210 … B-209→B-214 on Aug 7 (a parallel session's Trash Service legitimately took B-205).
- B-210 · 🟠 **Email delivery — Gmail API → `ridgecomaintenance@gmail.com` (address LIVE Aug 7).** `deliverDigestEmail` is a stub; digest + weekly Reviewer can't reach Brett. Path: OAuth client + refresh token for the RidgeCo Gmail, stored as Worker secrets (`GMAIL_CLIENT_ID/SECRET/REFRESH_TOKEN/SENDER`); Worker exchanges refresh→access token and calls Gmail `users.messages.send` (base64url RFC822). **No domain, no ESP** (fits security-first — it's Google, where the business already lives). ~500/day free-Gmail cap = ample for ops. **Scope internal-first:** digest + weekly review + overnight-agent output → Brett's inbox; owner/tenant/vendor + Hive/van sends stay MANUAL (draft-and-Brett-sends) until Brett flips them on (real customer sends are GATED per AUTONOMY_GUARDRAILS). Supersedes the earlier Resend/Cloudflare-Routing framing (those needed a domain). **Setup:** Brett creates the OAuth client + refresh token at a computer (~15 min); Claude writes the Worker send helper. Ties B-129 delivery, digest, B-214.
- B-211 · 🟠 **Independent verifier write-gate (`judge()`).** ✅ **BUILT + TESTED Aug 22.** `judge(env, call)` in worker.js right after the B-127/model-registry block, reuses `routeAI` (CHEAP tier) + `logTelemetry`. Returns `{verdict,confidence,reason,model_used,tier_used,escalated}`. **Fails closed on every path** — missing action/proposedChange, an explicit non-SAFE `riskClass` (structural backstop; GATED actions never reach an LLM verdict at all), a `routeAI` throw, unparseable/non-JSON model output, or confidence below the LOCKED 0.7 floor all resolve to `reject`; there is no code path that silently produces `approve`. New `POST /judge` on-demand test-drive endpoint (not in PUBLIC_PATHS → inherits the WORKER_SECRET gate, same convention as `/ops-review`/`/telemetry/log`) so Brett can exercise it before anything autonomous calls it, mirroring how B-127 shipped `GET /model-registry` standalone. `test/judge-write-gate.test.mjs` — 25 assertions (fail-closed defaults, riskClass backstop, confidence-floor edges at 0.5/0.7/0.99, non-numeric/out-of-range confidence coercion, markdown-fence JSON cleanup, telemetry-failure-never-flips-verdict), all passing, routeAI/logTelemetry mocked (no network I/O in the test, matching `model-routing.test.mjs` convention). **Not wired into a live write path** — same status as B-127 itself: nothing autonomous exists yet that judge() gates (Rung 2 is off; per BUILD_ORDER_v1.0's locked rule, no hand-edited worker.js ships until Phase-1 substrate + B-144 Quality Bar are done). It's ready the day a real Rung-1→Rung-2 write path lands — that call site is the next real step, not this build.
- B-212 · 🟡 **Golden-set regression gate on deploy.** 20–50 canonical cases re-run against the deployed Worker on push-to-main; pairs with `test-verified-builds`. Catches silent quality regressions before the live Hub. The earn-in that unlocks Rung-2 auto-ship.
- B-213 · 🟡 **Cloudflare Queues (free since Feb 2026) + D1** for durable overnight/telemetry pipeline. Sheets is slow/rate-limited; Queues (10k ops/day free, 24h retention) + D1 (SQL, free) make the overnight agent restart-safe and telemetry cheap. Keep Sheets as the human-facing DB; D1 = machine-facing. Infra for B-214.
- B-215 · 🔧 LIVE (Aug 8) · **AR aging — money owed, from QuickBooks.** Read-only `GET /ar/aging` (admin-gated): pulls open invoices from QBO, buckets by days-past-due (current/1-30/31-90/90+), rolls up by customer oldest-first. Shows on the BrettOS Command Center "Money owed" card (supersedes the Sheet tile when live; QB-Fault-guarded fallback). **Phase 2 (next, GATED):** one-tap reminder = QBO `SendInvoice` re-send for 30d+ — customer-facing money move, stays behind Brett's tap. Deployed `2026-08-08.2`.
- B-216 · 🟠 **Receipts → QuickBooks last-mile (preview-first).** The extract→queue→file pipeline already exists (`receiptExtract`/`Receipts_Queue`/`receipt-queue/approve`); the MISSING money step is posting an approved receipt to a QBO expense/bill + a `Receipts_Ledger` row for reconciliation. Money-WRITE ⇒ preview-first + Brett-gated (never auto-post). Saves 3-5 min/receipt + recovers lost deductions. Next build.
- B-214 · 🔧 LIVE (v1, Aug 7) · **Overnight Rung-1 Prepare agent (the "improve while I sleep" engine).** Scheduled Cowork task `trig_01Li7mADCLMXVamAFXvcdU1t`, **Tue/Fri 6am ET**, push notification. **v1 scope (honest to headless limits):** a fresh scheduled session has no PAT/WORKER_SECRET, so it can't deploy or run live smoke tests — instead it selects the top **SAFE-class, unblocked, well-specified** item and produces a **build-ready brief** (exact files/functions via CODEMAP + acceptance criteria + validation checklist + a labelled DRAFT implementation sketch), delivered via push for Brett's one-tap approval and fast **in-session** execution (where live-validate + deploy happen). Never touches GATED items (money/PII/auth) — it names them for Brett instead. This is Rung-1 of AUTONOMY_GUARDRAILS. **v2 (later):** draft the actual code once trusted. Surfaces via push today; B-210 email would add an inbox copy.
...
- B-230 · 🟡 **Small-item task system with connection-crawling** (Brett, Aug 22 — the "I want a ClickUp-like place for disjointed small to-dos" ask). Two halves: (1) a capture surface for small admin/to-do items that aren't tied to any one active project — don't build a new UI for this, it already exists (CAPTURE_INBOX.md/BACKLOG.md feeding the BrettOS Tasks sheet per B-091/CAP-024/CAP-025); (2) a periodic pass that reads everything captured there and surfaces items that actually connect — to each other, or to a bigger venture/build (e.g., three unrelated-looking small items that are all secretly about the cargo vans; or a cluster big enough to deserve its own project). Half 2 is a direct extension of two things already built/in-flight rather than a new system: the Second Brain semantic layer (B-133/B-134) and the Reuse-Radar clustering lens (B-131, which already finds "shared mechanism" connections across ventures — this reuses that lens against to-do text instead of tools). Proposed shape: keep capture as-is (type/voice/photo, zero new friction) → a scheduled pass (piggyback the existing Optimizer Scout Mon/Thu cadence) clusters open small items by shared entity/venture/keyword against Ventures + BACKLOG headings, flags any 2+-connection group or a load-bearing single item → present findings as a cluster/map view via `mcp__visualize` rather than a flat list (this is what Brett means by "the goal of the visualization tool"). Open design questions before building (don't guess — ask Brett): keyword/venture-tag matching (cheap, likely good enough at this corpus size) vs. embeddings (better recall, needs a vector store — see B-213's D1 infra); and whether a found connection auto-tags the item or only surfaces as a suggestion for Brett to confirm. Links: B-091, B-131, B-133, B-134, CAP-015, CAP-031 (first real content this would run against — the Cabin logistics list).
- B-231 · 🟡 **Event-triggered scheduling — replace polling with push/webhooks where a real event exists** (Brett, Aug 22, after the scheduled-task audit that produced Rule 7). Aug 22 pass turned off two of nine recurring Cowork scheduled tasks outright (Inspection Watcher, BrettOS Context Update Reminder — see SESSION_EFFICIENCY_PROTOCOL Rule 7) and kept six running on their existing time-based cadence: Morning contact-loss guard (daily), Michelle Dykes + Andrea's Cleaning invoice intake (2x/week each), Ridge Co Daily Scan Intake (daily), Optimizer Prepare + Scout/Reuse-Radar (2x/week each). Several of those are reacting to something that already IS an event (an email arriving, a file landing in Drive) but are currently polling for it on a timer instead — that's the real target of this item, not the low-frequency proactive ones (Optimizer Prepare/Scout, weekly data-removal) which have no natural external event to react to and are fine time-based.
  - **Candidates to convert, ranked by how well-matched the trigger is:** (1) Inspection Watcher — Brett wants this one specifically redesigned around Gmail push instead of hourly polling; (2) Michelle Dykes / Andrea's Cleaning invoice intake — same shape, react to "an email from this vendor arrived" instead of checking twice a week; (3) Morning contact-loss guard — currently guesses "8am, after the ~6:44am sync"; could react to "today's Contacts-sync report email arrived" instead of a fixed time; (4) Ridge Co Daily Scan Intake — Brett's own riff: could react to "a new file landed in the scan-intake Drive folder," which might fire 10x on a day he's scanning and zero times for days otherwise — exactly the shape a real event trigger should have. **Deliberately NOT proposed for conversion:** Optimizer Prepare/Scout (proactive research/audit, no external event to hang off) and the weekly data-removal cycle (same reasoning) — Brett flagged these as probably fine on a timer, and nothing here disagrees.
  - **The real mechanism (needs research before building, not assumed):** Gmail supports push notifications via `users.watch()` + a Cloud Pub/Sub topic — the mailbox change gets published, a subscriber receives it, no polling. Google Drive's API has an analogous `changes.watch` for file-level pushes. Both need a real always-on receiver, which a Cowork session is NOT (sessions aren't persistent servers) — Brett's Cloudflare Worker (`maintenance-hub`) is the one piece of infra that IS always-on, so it's the natural place to receive these pushes. **Two open questions to resolve before building, not decided here:** (a) can the Worker actually fire a Cowork scheduled task on demand from the outside — UNKNOWN, no evidence found either way, needs checking against current Anthropic docs (PAT-028) before assuming it's possible; (b) does "event-triggered" mean the push spins up a full Cowork agent run every time, or does the Worker do the deterministic part itself (parse the email, check the calendar) and only call out to Claude for the part that needs judgment (drafting the reply) — this changes the architecture a lot and should be Brett's call per case, not assumed uniform. Also note: Gmail watches expire every 7 days and need renewal — that renewal job has to live somewhere durable (the Worker again, likely a small cron there rather than a Cowork schedule).
  - Links: SESSION_EFFICIENCY_PROTOCOL Rule 7 (the audit that surfaced this), B-210 (Gmail API already being wired for send — same auth plumbing partly reusable for watch), the `maintenance-hub` Worker.
- B-136 · 🟡 ⏳ SMS door for the brain — once Twilio is fixed, an inbound route calls the same `handleAsk`; caller-ID whitelist = auth; adds "reply MORE" + dictated MMS voice notes (finishes B-052). **Twilio unblock IN PROGRESS — Aug 10, 2026: fixed the wrong sole-prop registration → Low-Volume Standard under Saint Thomas Ventures LLC (DBA Ridge Co). Business Profile APPROVED, brand registered, A2P campaign PENDING_REVIEW (Twilio quotes 1–3 biz days); SMS privacy + terms pages live at bmoremanagement.com/sms-privacy + /sms-terms. Account/brand/campaign SIDs in PRIVATE repo → `business-context/twilio-a2p-status.md`. (Was: blocked on Twilio setup fix.)**
...
- B-142 · ✅ **ridgeco-validate skill (autonomy prerequisite #3 — REVIEW GATE).** Adversarial built-vs-brief validator; the missing 7th "software-factory" role. Reports gaps by severity, fixes nothing; mandatory for money/customer/auth. Delivered July 23 as `ridgeco-validate.skill`. **Wiring into brett-flow as the actual step-5.5 gate completed Aug 23** — `brett-flow` SKILL.md's build flow now has a real numbered step 6 that names and calls `ridgeco-validate` between self-review and smoke-verify (the Quick Index line calling this "confirmed shipped" back on July 23 was premature — the skill existed but brett-flow never actually invoked it until this fix).
...
- B-144 · 🟠 **Quality Bar / Definition-of-Done rubric (DEFINE — the missing quality parameter).** ✅ **BUILT Aug 23.** `context/QUALITY_BAR_v1.0.md` — three change-class tables (Worker Endpoint / Hub Screen / Money Change), each a binary pass/fail checklist grounded in real documented failures (rule 6 header-name resolution, rule 18 node --check, rule 37 ensureColumns, UI_QA_CHECKLIST 132/134, the double-post/reconcile-never-auto-corrects invariants), not abstract style opinions. This is what ridgeco-validate and the future reviewer (B-146) score against — B-149's house-style checks are the greppable subset of it. Doc-only change, zero blast radius, no worker.js/index.html edit. Unblocks the BUILD_ORDER_v1.0 Phase-1 gate that was holding B-127/B-211 deployment.
...
- B-100 · ✅ SHIPPED Aug 18 (FEATURE_LOG rule 104) — Pre-triggered / dependency WOs (turnover cleaner). **Stale row corrected Aug 22** (was still shown 🟠 open despite shipping — see rule 104: Repairs+Paint open in parallel, Cleaning On Hold auto-releases). Still flagged 🔴 needs Brett's first live confirm as of Aug 22 — never verified since it shipped.
...
- B-227 · 🟠 Hybrid/materials-service vendor payments (locksmith trigger, Aug 19) — narrow slice of B-183 that does NOT need WO-splitting: let a WO carry 2+ Vendor_Bills rows (already possible via "Enter a bill by hand," no code change) and have `qbSendInvoice` combine every approved Invoice_Review row sharing one WO_ID into ONE customer invoice while still posting one separate QB Bill per vendor (already correct). Plus a `Vendor_Type` field (labor/materials_store/materials_hybrid) and a "ready to pay once customer invoice is Paid" surfacing on Who-to-Pay. Full plan + 5 open questions for Brett: `context/HYBRID_VENDOR_PAYMENTS_BUILD_BRIEF_v1.0.md`. Blocked on Brett setting `PAY_AUTH_CODE` (see B-217A) before any of this pays live. **Aug 20: Q1–Q5 answered (FEATURE_LOG 117). Phase 1 pushed (commit `b2c390f`, `2026-08-20.1`). Phase 3 — the actual combined-invoice gap — built + tested + ridgeco-validated (1 real 🔴 caught pre-push and fixed: duplicate-Bill risk on retry, see FEATURE_LOG 121), rebased onto Aug 21's tenant-WO-toggle push and pushed at `2026-08-20.4`, awaiting Brett's live Preview & Send test on a real 2-vendor WO before he trusts Confirm on a real job.** Q2 surfaced new scope: email-to-invoice ingestion (FEATURE_LOG 117b) — not started, needs its own design decision. Non-blocking follow-ups from Phase 3's validation pass: no concurrent-send lock, receipt re-attach-on-retry quirk (pre-existing pattern, not new).
...
**RIDGE CO — APPLIANCE & MATERIALS DELIVERY SYSTEM (B-218..B-223 — Brett's Aug-11 brain-dump; full spec in `DELIVERY_SYSTEM_BUILD_BRIEF_v1.0`)**
_Kills the single-contact bottleneck on deliveries: tenant is blind + the delivery-day call goes to Brett (busy) → missed deliveries. Phased so Phase 0–2 land value with NO Twilio; Phase 3+ needs the dedicated line. GATED class (PII + customer comms)._
- B-218 · 🔧 **SHIPPED (Aug 11, `2026-08-11.2`) — pending Brett live-verify. Delivery record + Hub "🚚 Deliveries" screen (Phase 0, SAFE).** `deliveries.html` + `Deliveries` tab (self-provisions) + `GET /deliveries`, `POST /delivery/add` (auto-creates+links the install WO), `POST /delivery/update`. Relay-to-tenant = `sms:` prefill (draft-first). Create-from-WO via URL prefill. See FEATURE_LOG rule 79. **REMAINING:** "Create delivery" button on the in-Hub WO detail modal (deep-link already works). Original spec: New `Deliveries` tab (order#, store, item+make/model, property/unit/tenant, delivery address, date/window, on-site contact [default tenant] + backup, notes, linked WOs, status). Standalone tool page + button on the WO detail modal; create-from-WO pre-fills, and a delivery with no WO **auto-creates** the install WO (two-way link). "Today's deliveries" Command-Center card + one-tap **tenant relay** (draft-first) — kills the 5am "decode the order# and hand-text the tenant" pain with no Twilio. Reuses `createWorkOrder`/`addRow`/`fetchTab`; `ensureColumns` (rule 37); WO key by `ID` header (rule 6).
- B-219 · 🟠 **Appliance-order email watcher (Phase 1, SAFE).** Extends the B-103 intake engine with a pluggable appliance-order parser on **`info@bmoremanagement.com`** (Ridge Co address swaps in via config later). Parses order confirmations / scheduled-date / narrowed-window / reschedule mail → creates/updates the Delivery record, links property/unit/tenant/WO by delivery-address match (`normalizeAddr`, queue on ambiguous, never dup-create). Idempotent on Order# + Message_ID; reuses `INTAKE_TOKEN`.
- B-220 · 🟠 **Dimension safety check + nameplate OCR (Phase 2, SAFE) — graduates B-170.** Compare the new appliance's dimensions to the outgoing unit's (nameplate make/model captured at the service visit via photo→OCR). Flag **both directions** (20→30 or 30→20) beyond tolerance (default >1" width on ranges/fridges/stacked-laundry, configurable) as a one-tap **confirm** — never a hard block. New-model dims from spec lookup / AI-with-human-confirm (treat AI spec as first-guess-to-verify per repair playbook §6). Feeds CAP-010 asset registry.
- B-221 · 🟠 **Dedicated Twilio deliveries line + routing brain + AI voice agent (Phase 3–4, GATED). Step 0 = stand up Twilio (unblocks B-136/137/156/158/162 + digest SMS).** ✅ **Step 0 IN REVIEW (Aug 10, 2026):** A2P brand + campaign submitted at Low-Volume Standard under the Ridge Co brand — campaign `PENDING_REVIEW`. Details/SIDs in PRIVATE `business-context/twilio-a2p-status.md`. A **dedicated** deliveries number (decided Aug 11 — isolates routing logic + protects tenant-msg reputation; ~$1–2/mo, registration shared under the Ridge Co brand). Inbound SMS parse (confirm/window/reschedule/en-route) on `/sms-inbound`. Disambiguation ladder for calls: 0 active → take a message + alert Brett; 1 → route direct; 2+ → AI agent asks delivery address/order# → match. Matched → AI answers driver Qs from the record (floor/access, scoped codes per B-055), bridges to the on-site contact, or cascades tenant → assignee → backup. Store only ever knows "call this number on arrival." Voice = ConversationRelay (`twilio-developer-kit` plugin). Reuses `sendSMS` chokepoint.
- B-222 · 🟠 **Delivery notification fan-out + calendar invites + en-route touchpoint (GATED comms).** On every delivery event notify tenant + Brett/assignee + backup + owner(awareness), EN/ES (reuse B-093/137/156). **Calendar (folds B-204):** entry on Brett's own calendar (no invite) + invites to tenant + backup + owner(optional/awareness); auto-updates on window-narrow. **En-route "stops away" touchpoint (Brett's Aug-11 add):** when a store sends a live en-route signal (SMS/email "N stops away"/tracking link), fire an **immediate tenant SMS** (in addition to the window notice) + **optional auto-call** to the tenant if they're the primary on-site contact + calendar bump to `Approaching`. Best-effort — not every store sends it; use fully when present. **Confirmation-of-receipt + damage-claim loop** after the window (ties B-190). **Delivery-exception protocol (§8.6, as-needed — NOT broadcast):** when a tenant reports "it won't install," triage → default **keep-and-fix** (tenant asks the crew to leave it on-site → we dispatch our own vendor to fix the site issue + install, spawning a child WO per B-223); **return & reorder only** for wrong-size-by-large-margin (~4"+) or can't-get-it-inside — hard rule: never left on the curb / in a hallway / blocking fire egress.
- B-223 · 🟠 **Related/sub work orders + combine-or-split invoicing (GATED money) — graduates B-096 + B-183.** Parent goal ("working refrigerator") with child WOs: diagnostic/service (Vendor A) + delivery/install (Vendor B). Support **both** one combined invoice (children roll up under the parent) **and** two separate invoices, chosen per-parent at send. Ties owner approval of the marked-up appliance purchase (B-126), invoice quality (B-190), NTE thresholds (B-179), asset-lifecycle / repair-vs-replace register (B-194), receipt→QB of the order confirmation (B-216).

- B-226 · 🔴 **Inspection Scheduler — new venture line (rental-customer batch inspections + AMSCRE routing).** New standalone `inspect.html`/`inspect-book.html` pair inside the existing Hub stack (mirrors Trash Service B-205 architecture — own Worker endpoints + self-provisioning `Insp_*` Sheet tabs, no new infra). Researched + rejected Cal.com/Easy!Appointments as a backbone (can't run on Cloudflare Workers, would need a second DB/server). Two intake lines sharing one engine: (1) a new PM customer's SFH+multifamily annual inspections — Brett defines availability + blackout rules on the Hub, batches multi-unit sites into one visit window with per-tenant slots, self-serve booking link + reply-capable SMS, auto-nudges non-responders, texts Brett to approve-partial-fill or reschedule; (2) AMSCRE one-off inspections — zip-based pending-cluster queue so Brett can batch a route before committing, manual release or auto-release-solo after a hold window. Full spec: `INSPECTION_SCHEDULER_BUILD_BRIEF_v1.0.md`. **Phased** (checkpoint each): P1 data model + admin onboarding + availability/blackout UI — ✅ SHIPPED Aug 18 (FEATURE_LOG rule 110, `inspect.html` live) + P1 follow-up (blackout date ranges/times, bulk paste, bulk property/unit import, search/collapse for scale) — ✅ SHIPPED Aug 18 (FEATURE_LOG rule 113), pending Brett's first live pass on both → **P2 window/slot engine + outreach + booking page (NEXT)** → P3 auto-nudge + Brett SMS-approve + AMSCRE queue → P4 inbound reply/AI-draft handling → P5 (later) AMSCRE email intake, real geocoding, Calendar sync.
<!-- QUICK-INDEX:END -->
...
| B-226 | Ridge Co | Inspection Scheduler — new venture line (rental-customer batching + AMSCRE routing) | 🟡 Phase 1 SHIPPED (data model + admin onboarding + availability/blackout UI, FEATURE_LOG rule 110) + follow-up SHIPPED (blackout ranges/times, bulk paste, bulk property/unit import, search/collapse, FEATURE_LOG rule 113) — pending Brett's first live pass. Phase 2 (outreach + booking page) next. See `INSPECTION_SCHEDULER_BUILD_BRIEF_v1.0.md`. |
...
| B-140 | 🟢 | **Staging/preview Worker lane** — autonomy prerequisite #1 (isolation) | ✅ **BUILT July 23, CONFIRMED LIVE Aug 22** — SEPARATE Worker `maintenance-hub-staging` (chosen over a same-Worker preview because secrets are global + the `staging-` isolation code doesn't exist in worker.js). Own secrets: SHEET_ID=staging sheet `16PCD3t…`, FRESH GOOGLE_SA_KEY, own WORKER_SECRET, NO QB. Aug 22: `curl .../health` → 200, `sheet_tail:0H6dFY` (staging sheet confirmed, not prod's), real tab counts. Fixes rule 18. From CAP-029. |
| B-141 | 🟢 | **Smoke-test harness / self-check** — autonomy prerequisite #2 (self-verification) | ✅ **DONE Aug 22.** The July-23 SNAG (Cloudflare production-branch setting not pointed at `staging`) was already fixed by the time this session checked — Brett confirmed via dashboard screenshot, `/health` responds 200 live. Built `scripts/smoke-staging.mjs` — curl-asserts `/health` (200, ok:true, correct sheet_tail, all 4 expected tabs present as numbers) + `/version` (200, version string) against the real staging Worker. 10/10 passing. **Scope note:** only covers what's curlable with zero setup (no WORKER_SECRET, no seeded PIN/share-token record) — broader endpoint coverage is B-145 (golden-path tests + seeded fixtures), not this item. From CAP-029. |
| B-127 | 🟠 | **Model routing layer** — secure DIY multi-model router | ✅ **BUILT + TESTED Aug 22, NOT DEPLOYED (intentional).** `routeAI(env,job)` + `MODEL_REGISTRY` + `callGemini`/`callClaude` adapters + `GET /model-registry`, reuses existing `logTelemetry`→`Ops_Telemetry` (columns already existed from B-128). `test/model-routing.test.mjs` 17/17 passing. Did NOT migrate the ~6 existing direct-Claude call sites onto it (per-flow blast-radius call, not this build). **Caught + fixed mid-session: the brief's `gemini-2.0-flash` was already shut down by Google (June 1, 2026) — swapped to `gemini-2.5-flash-lite`, comment left to bump to `gemini-3.1-flash-lite` before Google retires 2.5 Flash-Lite Oct 16, 2026.** Not deployed because (a) `BUILD_ORDER_v1.0`'s own locked rule — no hand-edited worker.js ships until Phase-1 substrate is done, and B-144 (Quality Bar) still isn't built, (b) nothing in the live app calls `routeAI()` yet — pure plumbing, real cost is $0 until a job type is wired in. Brett added `GEMINI_API_KEY` to both `maintenance-hub` + `maintenance-hub-staging` (Prepay billing, $10 min — expected, not a bug). **🔴 NOT PUSHED to GitHub — exists only in the session-that-built-it's local clone. Verify with `grep -n "routeAI" worker.js` before assuming it's on `main`; if absent, needs BRETT_GH_PAT to push or a rebuild from this note + CURRENT.md.** Brief: `MODEL_ROUTING_BUILD_BRIEF_v1.0`. |
...
| B-144 | 🟠 | **Quality Bar / Definition-of-Done rubric** — DEFINE (the quality parameter) | ✅ BUILT Aug 23 — `QUALITY_BAR_v1.0.md`. Per change-class "what good is" checklists; what validator+reviewer score against. From CAP-029. |
...
| B-100 | ✅ | Pre-triggered / dependency WOs (turnover cleaner) | SHIPPED Aug 18 via the Turnover trigger (FEATURE_LOG rule 104) — Repairs+Paint open immediately in parallel, Cleaning created On Hold and auto-releases once both finish or the day before target move-in. 🔴 Never Brett-confirmed live. |
