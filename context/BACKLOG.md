...
- B-127 · 🟠 **TOP-LEVEL PRIORITY GOING FORWARD (Brett, Aug 13).** Model-routing layer — secure DIY multi-model router in Worker (cheap-by-default + escalate; Gemini+Claude direct; PAT-031). Brief: `MODEL_ROUTING_BUILD_BRIEF_v1.0`. Deferred Aug 13 (API calls turned out NOT to be Brett's big burn — the session limits were; fixed separately via SESSION_EFFICIENCY_PROTOCOL). Keep queued near the top; build in its own focused chat when Brett is ready. `routeAI` reuses `logTelemetry` (B-128).
