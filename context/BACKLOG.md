...
- B-127 · WO notes & visibility model — Entry_Notes / Owner_Notes / Vendor↔Admin thread / Admin-only / Hold_Reason + per-role view enforcement [Ridge Co Hub] — was specced as B-104; renumbered to B-127 because the July 21 capture batch assigned B-104 to the Cabin STR cleaner item (which keeps it). Spec titles + docs updated to B-127; git commit messages before this point still say B-104 (history, left as-is). Nothing in code depends on the ID.
<!-- QUICK-INDEX:END -->
