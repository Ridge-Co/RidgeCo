...
- B-133 · 🟠 Second Brain — **multi-role Ask agent** (v1.1): PIN-gated `ask.html` (EN/ES) + `POST /ask` (direct Worker call, NOT a Cowork session). **P0 = identity-scoped authorization** — roles admin/vendor/driver; each asker scoped to its allow-set (vendor: own WOs + assigned-property codes + own pay; driver: pallet drop-network + assigned-site gate codes + own pay). FACT (scoped live Sheets) / SEMANTIC (Audience-filtered Brain) / CAPTURE. Read+capture only; audit every sensitive answer. Seed Brain now. The seed of Brett's vendor/driver assistant (ties vendor.html, B-132, B-055). Brief: `SECOND_BRAIN_QUERY_BUILD_BRIEF_v1.1`.
