...
- B-211 · 🟠 **Independent verifier write-gate (`judge()`).** Cheap-model LLM-as-judge returns `{verdict,confidence,reason}` before any consequential write; fails closed → escalate to Brett. The mechanism that lets autonomy graduate Rung-1→Rung-2 (AUTONOMY_GUARDRAILS). Direct fix for both past burns. Reuses B-127 routing (cheap tier). GATED-adjacent — build with Brett.
