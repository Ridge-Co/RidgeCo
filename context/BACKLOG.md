...
- B-227 · 🟡 Repo-wide sweep of the label-wraps-hidden-file-input tap-target bug (Aug 22 — same root cause as B-205's Aug 22 "Add photos" fix, FEATURE_LOG rule 132, but that pass only touched Trash Service). Same pattern confirmed present in `wo.html` (3 photo/receipt buttons), `scope-creator.html` (2: handwriting upload, before-photos), and the shared `inputAttrs` helper used by `index.html`/`tenant.html`/`owner.html`/`vendor.html`'s receipt/photo pickers. Not yet fixed anywhere but Trash Service — Brett said this is "a behavior I don't want to repeat," which is exactly why it's worth the sweep rather than fixing it one report at a time.
- B-228 · ⏳ Blocked — Brett's "add it to a checklist project" ask (Aug 22, re: B-227/rule 132). Unclear whether he means a ClickUp list (ClickUp is connected this session) or the Hub's own Wishlist/Dev Log checklist mechanism (line ~18 above). Asked in plain text (not the question widget — Brett's standing mobile instruction). File under whichever he confirms.
