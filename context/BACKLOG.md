...
- B-205 · 🟠 **Email delivery path (FIXES EXISTING — top priority).** `deliverDigestEmail` is a stub; digest + weekly Reviewer can't reach Brett. Ship in layers: (a) **Cloudflare Email Routing `send_email` binding** — free, sends to VERIFIED recipients only → perfect for self-digests/alerts to brett@ NOW (needs Brett to add the binding + verify his address in the CF dashboard); (b) **Resend** (100/day, 3k/mo free, custom domain, plain `fetch` from the Worker) for owner/tenant/vendor-facing sends later (needs a `RESEND_KEY` secret + domain verify); (c) Cloudflare Email Service binding = own-the-Worker endgame (paid beta ~$5/mo). **MailChannels is dead (EOL 2024); SendGrid free tier killed.** Unblocks the "look in once in a while" model. Ties B-129 delivery, digest, B-209.
- B-206 · 🟠 **Independent verifier write-gate (`judge()`).** Cheap-model LLM-as-judge returns `{verdict,confidence,reason}` before any consequential write; fails closed → escalate to Brett. The mechanism that lets autonomy graduate Rung-1→Rung-2 (AUTONOMY_GUARDRAILS). Direct fix for both past burns. Reuses B-127 routing (cheap tier). GATED-adjacent — build with Brett.
- B-207 · 🟡 **Golden-set regression gate on deploy.** 20–50 canonical cases re-run against the deployed Worker on push-to-main; pairs with `test-verified-builds`. Catches silent quality regressions before the live Hub. The earn-in that unlocks Rung-2 auto-ship.
- B-208 · 🟡 **Cloudflare Queues (free since Feb 2026) + D1** for durable overnight/telemetry pipeline. Sheets is slow/rate-limited; Queues (10k ops/day free, 24h retention) + D1 (SQL, free) make the overnight agent restart-safe and telemetry cheap. Keep Sheets as the human-facing DB; D1 = machine-facing. Infra for B-209.
- B-209 · 🟠 **Overnight Rung-1 Prepare agent (the "improve while I sleep" engine).** Scheduled task that takes the top greenlit item, drafts the full change + runs `test-verified-builds` + `ridgeco-validate`, and leaves it ready for Brett's one-tap morning approval — **never deploys** (AUTONOMY_GUARDRAILS Rung 1). Unblocked now the guardrail is locked; needs B-205 (delivery) to surface output. This is the core of Brett's Aug-7 autonomy ask.
