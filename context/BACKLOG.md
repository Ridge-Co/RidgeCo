...
- B-128 · Job telemetry spine — `Ops_Telemetry` tab + `logTelemetry` chokepoint + secret-gated `POST /telemetry/log` for Cowork/skill rows (dual-source: Worker + Cowork). Prerequisite for the whole Optimizer — nothing downstream runs on judgment once this ships. **Brief: `TELEMETRY_SPINE_BUILD_BRIEF_v1.0` (graduated Aug 7).** Folds in 2 hardening adds: H1 un-gameable-verifier rule (test-verified-builds/ridgeco-validate), H2 Optimizer "stuck-pattern" Review step. Ships BEFORE B-127; `routeAI` reuses the same helper.
